...
const MARKER = fileURLToPath(new URL("./.dsh-debug.log", import.meta.url));
function dbg(message) {
	try {
		appendFileSync(MARKER, `${new Date().toLocaleTimeString()} ${message}\n`, "utf8");
	} catch (error) {
		try {
			appendFileSync("C:\\Users\\86156\\AppData\\Local\\Temp\\dsh-plugin-debug.log", `${new Date().toLocaleTimeString()} dbg-fallback-error: ${error.message}\n`, "utf8");
		} catch {}
	}
}
 ...
 * the toast bottom-right on scaled displays.
 */
...
export function showPopup({ title, body, seconds = 6 }) {
	...
	try {
		...
		dbg(`showPopup threw: ${error.message}`);
	}
}
//#endregion
...
export function apply(ctx, config = {}) {
	...
	dbg(`root.on available: ${typeof ctx.root?.on === "function"}`);
	const notifier = createPopupNotifier({ settings: () => current });
	const disposeEvents = ctx.root.on("session/event", (session, event) => {
		dbg(`session/event: ${event?.type} (${session?.id})`);
		notifier.onSessionEvent(session?.id, event);
	});
	const disposeStatus = ctx.on("agent/status", ({ agent, status }) => {
		dbg(`agent/status: ${status} (${agent?.id})`);
		if (status === "idle") notifier.onAgentIdle(agent?.id);
	});
	dbg(`hooks before test: ${ctx.root.events?._hooks?.["session/event"]?.length ?? "?"}`);
	// SELF-TEST: manually emit a session event to verify the listener fires
	ctx.root.events.emit("session/event", { id: "test-session" }, { type: "test/self-check", data: {} });
	dbg("self-test emit done");
	dbg(`hooks after test: ${ctx.root.events?._hooks?.["session/event"]?.length ?? "?"}`);
	dbg("listeners wired");
	ctx.effect(() => {
		disposeEvents();
		disposeStatus();
