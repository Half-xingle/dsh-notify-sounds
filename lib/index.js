...
/** SAFE diagnostic: file writes only, never emits events. */
const MARKER = fileURLToPath(new URL("./.dsh-events.log", import.meta.url));
...
export function showPopup({ title, body, seconds = 6 }) {
	...
	try {
		...
		child.on("error", (error) => {
			mark(`spawn error: ${error.message}`);
		});
		child.on("exit", (code) => {
			mark(`popup exit: ${code}`);
		});
		child.unref();
	} catch (error) {
		mark(`showPopup threw: ${error.message}`);
	}
}
//#endregion
...
export function apply(ctx, config = {}) {
	...
	mark("apply running, wiring listeners");
