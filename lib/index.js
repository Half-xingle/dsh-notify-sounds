...
//#region native popup (fixed-signature one-shot)
import { tmpdir } from "node:os";
import { join } from "node:path";
import { writeFileSync } from "node:fs";

/**
 * Fixed temp paths for the popup script + payload. The SCRIPT never changes,
 * so the spawned command line (`powershell -File <script>`) is CONSTANT —
 * anti-virus "remember my choice" rules match it once and never prompt again.
 * (An -EncodedCommand payload would vary the command line per popup and
 * re-trigger prompts every time.)
 */
export function popupPaths() {
	return {
		script: join(tmpdir(), "dsh-notify-sounds-popup.ps1"),
		payload: join(tmpdir(), "dsh-notify-sounds-popup.json")
	};
}

/**
 * The constant popup script: reads the payload JSON from the fixed path and
 * renders one WinForms toast. No Add-Type -TypeDefinition DPI prelude (C#
 * compilation via csc silently kills hidden spawns); the registry-based scale
 * compensation keeps the toast bottom-right on scaled displays.
 */
export function buildPopupScript() {
	const { payload } = popupPaths();
	const single = (value) => "'" + String(value).replace(/'/g, "''") + "'";
	return `
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
$ErrorActionPreference = 'SilentlyContinue'
$payloadPath = ${single(payload)}
$data = $null
try { $data = Get-Content $payloadPath -Raw -Encoding UTF8 | ConvertFrom-Json } catch {}
if ($null -eq $data) { exit }
$title = [string]$data.title
$body = [string]$data.body
$seconds = [Math]::Max(2, [Math]::Min(30, [int]$data.seconds))
...
$form.Left = [int](($wa.Right - $w - 18) * $scale)
$form.Top = [int](($wa.Bottom - $h - 18) * $scale)
...
$lblTitle.Font = New-Object System.Drawing.Font('Microsoft YaHei UI', (10 * $scale), [System.Drawing.FontStyle]::Bold)
$lblTitle.AutoSize = $true
$lblTitle.Location = New-Object System.Drawing.Point([int](18 * $scale), [int](12 * $scale))
$lblTitle.MaximumSize = New-Object System.Drawing.Size([int](($w - 36) * $scale), [int](30 * $scale))
$lblTitle.BackColor = [System.Drawing.Color]::Transparent
$lblBody = New-Object System.Windows.Forms.Label
$lblBody.Text = $body
$lblBody.ForeColor = [System.Drawing.Color]::FromArgb(185, 188, 195)
$lblBody.Font = New-Object System.Drawing.Font('Microsoft YaHei UI', 9)
$lblBody.AutoSize = $true
$lblBody.Location = New-Object System.Drawing.Point([int](18 * $scale), [int](40 * $scale))
$lblBody.MaximumSize = New-Object System.Drawing.Size([int](($w - 36) * $scale), [int](44 * $scale))
...
/**
 * The constant spawn arguments. `-File <fixed script>` keeps the whole
 * command line identical across popups (payload lives in the JSON file).
 */
export function popupSpawnArgs() {
	return [
		"-NoProfile",
		"-WindowStyle",
		"Hidden",
...
/** Ensure the constant popup script file exists (writes it once). */
export function ensurePopupScript() {
	const { script } = popupPaths();
	try {
		writeFileSync(script, buildPopupScript(), "utf8");
	} catch {}
	return script;
}
...
export function showPopup({ title, body, seconds = 6 }) {
	try {
		...
		ensurePopupScript();
		const child = spawn("powershell.exe", popupSpawnArgs(), {
			windowsHide: true,
			stdio: "ignore"
		});
		child.on("error", () => {});
...
/**
 * Plugin body: register the settings section, spawn the popup helper, and wire
 * the native-popup listeners. `session/event` must be observed on the root
 * context (session observers live on the session scope chain); the listeners'
 * lifecycle is still tied to this plugin via ctx.effect.
...
export function apply(ctx, config = {}) {
	...
	const disposeEvents = ctx.root.on("session/event", (session, event) => {
	...
	ctx.effect(() => {
		disposeEvents();
		disposeStatus();
	}, "notify-sounds: native popup listeners");
}
