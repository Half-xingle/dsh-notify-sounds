...
export function showPopup({ title, body, seconds = 6 }) {
	console.log(`[dsh-notify-sounds] showPopup: ${title}`);
	try {
		...
		child.on("error", (error) => {
			console.log(`[dsh-notify-sounds] spawn error: ${error.message}`);
		});
		child.on("exit", (code) => {
			console.log(`[dsh-notify-sounds] popup exit: ${code}`);
		});
		child.unref();
	} catch (error) {
		console.log(`[dsh-notify-sounds] showPopup threw: ${error.message}`);
	}
}
//#endregion
...
export function apply(ctx, config = {}) {
	...
	const disposeEvents = ctx.root.on("session/event", (session, event) => {
		console.log(`[dsh-notify-sounds] session/event: ${event?.type} (${session?.id})`);
		notifier.onSessionEvent(session?.id, event);
	});
	const disposeStatus = ctx.on("agent/status", ({ agent, status }) => {
		console.log(`[dsh-notify-sounds] agent/status: ${status} (${agent?.id})`);
