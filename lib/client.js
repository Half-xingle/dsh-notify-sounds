 ...
 *
 * Settings: read/written through `ctx.settingsScope` bound to the
 * `notify-sounds` namespace (official pattern, dsh ≥ 0.1.1-rc.2). The host
 * half registers the same namespace via installSettingsSection, so the card's
 * toggles land in the user settings document ($DSH_HOME/settings.yaml) and
 * the host popup gating reads the SAME values — one source of truth. On a
 * remote (non-loopback) browser the settings transport is read-only, so the
 * card falls back to local defaults and writes are no-ops.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Fallback defaults (used while the settings scope is not ready / remote read-only). */
		...
		/**
		 * Observable settings store (uSES-compatible: getSnapshot + subscribe)
		 * over one bound settings scope.
		 *
		 * The scope snapshot exposes the layered document
		 * (`value` = resolved, `base` = composition, `user` = the document's own
		 * layer); only `user` entries are writable, and writes go through the
		 * scope's revision-fenced transport. The merged snapshot cached here
		 * keeps a STABLE reference between scope changes (required by
		 * useSyncExternalStore callers).
		 */
		function createSettingsScopeStore(scope) {
			...
			const merge = () => {
				...
				lastSnap = snap;
				const value = snap.status === "ready" && snap.value !== void 0 ? snap.value : {};
			...
			const scopeUnsub = scope.subscribe(() => {
				merge();
				notify();
			});
			...
			return {
				...
				set(field, value) {
					safeWrite(() => scope.set(field, value));
				},
				resetField(field) {
					safeWrite(() => scope.unset(field));
				},
				resetAll() {
					const user = scope.getSnapshot().user;
					if (user !== void 0 && user !== null && typeof user === "object") {
						for (const field of Object.keys(user)) safeWrite(() => scope.unset(field));
					}
				},
				/** Release the scope subscription (plugin unload). */
				dispose() {
					scopeUnsub();
				}
			};
		...
		var NotifyRuntime = class {
			...
			diffTodos(id, entry) {
				...
				if (newlyCompleted.length > 0) {
					...
					this.scheduleTodoToast(id, entry, last.content, done, total);
				}
				this.todoPrev.set(id, next);
			}
			/** Fire one aggregated progress toast, or fold the completion into a pending one. */
			scheduleTodoToast(id, entry, content, done, total) {
			...
			onList() {
				...
				for (const id of snap.ids ?? []) {
					...
					// NOTE: the sessions.list snapshot names this field parentId
					// (projectList maps parentSessionId -> parentId).
		...
		function apply(ctx) {
			...
			const scope = ctx.settingsScope.bind({ namespace: "notify-sounds" });
			const store = createSettingsScopeStore(scope);
			ctx.effect(() => () => store.dispose(), "notify-sounds: settings scope adapter");
		...
		exports.createSettingsScopeStore = createSettingsScopeStore;
