	...
	factory: (require) => {
		...
		var NotifyRuntime = class {
			...
			constructor(ctx, store) {
				...
				// step-progress tracking state
				this.stepSessionId = null;
				this.stepDisposer = null;
				this.stepLast = null;
				this.stepTracked = false;
				...
				ctx.on("connection/reset", () => {
					...
					// re-attach step tracking to the still-current session; the
					// session object survives a reconnect (it resyncs), so without
					// this the progress toasts would never come back.
					const sessions = this.ctx.get("sessions");
					const current = sessions?.list.getSnapshot().current ?? null;
					this.attachStepTracking(current);
				});
				installAudioUnlock();
			}
			...
			/**
			 * Attach step-progress tracking to the currently open session
			 * (binding exists only for open/staged sessions). Steps come from the
			 * conversation snapshot's assistant-message nodes; only advances while
			 * the session is running are reported, from `stepThreshold` on.
			 */
			attachStepTracking(sessionId) {
				if (sessionId === this.stepSessionId) return;
				this.resetStepTracking();
				this.diag.attached = sessionId ?? null;
				this.diag.subscribed = false;
				if (sessionId === null || sessionId === void 0) return;
				const sessions = this.ctx.get("sessions");
				const session = sessions?.binding?.(sessionId)?.session;
				if (session === void 0) {
					this.diagAttempt(`step: binding("${sessionId}") unavailable`);
					return;
				}
				this.stepSessionId = sessionId;
				this.stepDisposer = session.subscribe(() => this.onConversation());
				this.stepTracked = false;
				this.stepLast = null;
				this.diag.subscribed = true;
				this.diagAttempt(`step: attached to "${sessionId}"`);
			}
			/** Reset step tracking (disconnect, session switch, teardown). */
			resetStepTracking() {
				if (this.stepDisposer !== null) {
					try {
						this.stepDisposer();
					} catch {}
					this.stepDisposer = null;
			...
			/** Conversation snapshot fold: detect step advances while running. */
			onConversation() {
				const sessions = this.ctx.get("sessions");
				if (sessions === void 0 || this.stepSessionId === null) return;
				const session = sessions.binding?.(this.stepSessionId)?.session;
				if (session === void 0) return;
				const snapshot = session.getSnapshot();
				this.diag.lastSnapshot = {
					running: snapshot.running,
					nodes: (snapshot.nodes ?? []).length,
					partialStep: snapshot.partial?.step ?? null
				};
				if (snapshot.running !== true) {
					this.diag.lastGate = "not running";
					for (const listener of [...this.diagListeners]) listener();
					return;
				}
				this.diag.lastGate = "running";
				let maxTurn = 0;
				let maxStep = 0;
				for (const node of snapshot.nodes ?? []) {
					// AssistantMessageNode carries the per-step location; its kind
					// discriminant is "assistant" (see dsh-client-runtime types).
					if (node?.kind !== "assistant") continue;
					const turn = Number(node.turn) || 0;
					const step = Number(node.step) || 0;
					if (turn > maxTurn || (turn === maxTurn && step > maxStep)) {
						maxTurn = turn;
						maxStep = step;
					}
				}
				const current = maxStep > 0 ? { turn: maxTurn, step: maxStep } : null;
				if (!this.stepTracked) {
					// first observation while running: record baseline, no toast
					this.stepLast = current;
					this.stepTracked = true;
					this.diagAttempt(`step: baseline (turn ${maxTurn}, step ${maxStep}, ${(snapshot.nodes ?? []).length} nodes)`);
					return;
				}
				const last = this.stepLast;
				if (current === null || last === null) {
					this.stepLast = current;
					return;
				}
				const advanced = current.turn > last.turn || (current.turn === last.turn && current.step > last.step);
				if (!advanced) return;
				this.stepLast = current;
				const threshold = Number(this.settings.stepThreshold) || 3;
				if (current.step < threshold) return;
				const list = sessions.list.getSnapshot();
				const entry = list.byId?.[this.stepSessionId];
				const title = this.titleOf(this.stepSessionId, entry);
				this.notify(this.settings.notifSteps, {
					title: `DSH · ${title}`,
					body: `第 ${current.step} 步完成`,
					tag: `dsh-notify-steps-${this.stepSessionId}`
				});
			}
			...
			onList() {
				...
				// step progress follows the selected session (must be open/staged)
				this.attachStepTracking(snap.current ?? null);
			}
		...
		function NotifyCard(props) {
			...
			const debugLines = [
				`step: ${diag.attached ?? "—"} subscribed=${diag.subscribed ? "yes" : "no"}`,
			...
			return react.createElement("li", { style: cardStyle.card }, [
				...
				group("notifications", "notifGroup", [
					...
					row("notifSteps", t("notifSteps")),
					react.createElement("div", { style: cardStyle.row, key: "threshold" }, [
						react.createElement("span", { style: cardStyle.label, key: "label" }, t("stepThreshold")),
						react.createElement("input", {
							key: "number",
							type: "number",
							min: 1,
							...
							value: value.stepThreshold ?? DEFAULT_SETTINGS.stepThreshold,
							onChange: (event) => props.setField("stepThreshold", Math.max(1, Math.min(20, Math.round(Number(event.target.value) || 1))))
						})
					]),
		...
		const en = {
			...
			notifSteps: "Step progress",
			stepThreshold: "Notify from step",
